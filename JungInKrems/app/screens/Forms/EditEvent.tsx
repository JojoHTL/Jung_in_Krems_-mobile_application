...
function EditEvent({}){
    ...
    const GetEvent = async () => {
        const response = await axios.get('http://'+server+'/api/events/'+eventId);
        console.log('edit event data fetched');
        setData(response.data);

    }

    const getEventId = async () => {
        const Id = await AsyncStorage.getItem('EVENT_ID');
        if(Id!==null){
            ...
            <ScrollView>
                ...
                <View style={styles.inputmargin}>
                    ...
                    <View style={styles.inputcontainer}>
                        ...
                        <TextInput 
                        style = {styles.inputstyle}
                        placeholder = {data.WEBSITE_URL == null ? ('Link (optional)') : (data.WEBSITE_URL)}
                        placeholderTextColor={'#A8A8A8'}
                        onChangeText={setUrl}
                        value={url}
                        />
                    ...
                    <View style={styles.inputcontainer}>
                        ...
                        <TextInput 
                        style = {styles.inputstyle}
                        placeholder = {data.PHONE_NR == null ? ('Telefon (optional)') : (data.PHONE_NR)}
                        placeholderTextColor={'#A8A8A8'}
                    ...
                    <Pressable style={styles.createcontainer} onPress={() => [CreateEvent(),navigation.navigate('EventsMain')]}>
                        <Text style={styles.createtext}>{'Bestätigen'}</Text>
                    </Pressable>
                </View>
            </ScrollView>
        </SafeAreaView>
